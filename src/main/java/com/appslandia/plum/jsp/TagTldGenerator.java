...
public class TagTldGenerator {
  ...
  public static void main(String[] args) throws Exception {
    ...
    generateTag(ChoiceGroupTag.class, sb, dynClasses, notDynClasses, warnings);
    ...
    generateTag(FmtConstTag.class, sb, dynClasses, notDynClasses, warnings);
    ...
    generateTag(FmtStringTag.class, sb, dynClasses, notDynClasses, warnings);
    generateTag(TestViewErrorTag.class, sb, dynClasses, notDynClasses, warnings);

    generateTag(WrapUITag.class, sb, dynClasses, notDynClasses, warnings);
    ...
    generateTag(PagerTag.class, sb, dynClasses, notDynClasses, warnings);
    generateTag(SortThTag.class, sb, dynClasses, notDynClasses, warnings);
  ...
  public static void generateTag(Class<?> tagClass, TextBuilder sb, List<Class<?>> dynClasses,
    ...
    attributeProps.sort((p1, p2) -> {
      var a1 = p1.getWriteMethod().getDeclaredAnnotation(Attribute.class);
      var a2 = p2.getWriteMethod().getDeclaredAnnotation(Attribute.class);
    ...
    for (PropertyDescriptor pd : attributeProps) {
      ...
      var attribute = pd.getWriteMethod().getDeclaredAnnotation(Attribute.class);

      var attrName = attribute.name();
      if (attrName.isEmpty()) {
        attrName = StringUtils.firstLowerCase(pd.getName(), Locale.ENGLISH);
      }
      ...
      if (attribute.rtexprvalue()) {
        attrDesc.append(";rtexprvalue");
      } else {
        attrDesc.append(";rtexprvalue=false");
      }

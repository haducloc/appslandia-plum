 ...
 *
 */
public class BrotliResponseWrapperTest {
  ...
  public void test() {
    ...
    var wrapper = new BrotliResponseWrapper(response, 10, new MemoryStream(32));
  ...
  public void test_brotli() {
    ...
    try {
      ...
      Assertions.assertEquals("br", ce);

      var content = response.getContent().toString(StandardCharsets.UTF_8);
      Assertions.assertNotEquals("12345678901", content);

      var actualContent = TestUtils.unbrotliToString(response.getContent());
